...
def get_dosage_string(med: Concept, next_med: Optional[Concept], text: str) -> str:
    ...
    sents = re.findall(r"[^\s][^\n]+", text[med.start:next_med.start]) if next_med is not None else \
        re.findall(r"[^\s][^\n]+", text[med.start:])
...
class NoteProcessor:
    ...
    def add_dosages_to_concepts(self, concepts: List[Concept], note: Note) -> List[Concept]:
        """
        ...
        for ind, concept in enumerate(concepts):
            next_med_concept = concepts[ind + 1] if len(concepts) > ind + 1 and \
                                                    concepts[ind + 1].category == Category.MEDICATION else None
    ...
    def debug(self, mode: DebugMode = DebugMode.PRELOADED, code: Optional[int] = 0) -> (List[Concept], Dict):
        """
