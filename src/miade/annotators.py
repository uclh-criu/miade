...
def load_allergy_type_combinations(filename: str) -> Dict:
    ...
    df["reaction_id_name"] = list(
        zip(df["adverseReactionId"], df["adverseReactionName"])
    )

    # Set (allergenType, adverseReactionType) as the index and convert to dictionary
    result_dict = df.set_index(["allergenType", "adverseReactionType"])[
        "reaction_id_name"
    ].to_dict()
...
def get_dosage_string(med: Concept, next_med: Optional[Concept], text: str) -> str:
    ...
    """
    sents = sent_regex.findall(
        text[med.start : next_med.start] if next_med is not None else text[med.start :]
    )
...
def calculate_word_distance(
    start1: int, end1: int, start2: int, end2: int, note: Note
) -> int:
    """
    ...
    if start1 > end1 or start2 > end2:
        return (
            -1
        )  # Invalid input: start index should be less than or equal to the end index
...
class Annotator:
                ...
                if concept.start >= paragraph.start and concept.end <= paragraph.end:
                    ...
                    if concept.meta is not None:
                        if (
                            ...
                            or paragraph.type == ParagraphType.imp
                        ):
                            ...
                            for meta in concept.meta:
                                if (
                                    meta.name == "relevance"
                                    and meta.value == Relevance.IRRELEVANT
                                ):
                                    log.debug(
                            ...
                            for meta in concept.meta:
                                ...
                                if (
                                    meta.name == "substance_category"
                                    and meta.value == SubstanceCategory.IRRELEVANT
                                ):
                                    log.debug(
                                        f"Converted {meta.value} to "
    ...
    @staticmethod
    def deduplicate(
        concepts: List[Concept], record_concepts: Optional[List[Concept]]
    ) -> List[Concept]:
        ...
        for concept in concepts:
            if concept.id is not None and (
                concept.id in record_ids or concept.id in existing_concepts
            ):
                log.debug(
                    f"Removed concept ({concept.id} | {concept.name}): concept id exists in record"
                )
            # check name match for null ids - VTM deduplication
            elif concept.id is None and (
                concept.name in record_names
                or concept.name in existing_concepts.values()
            ):
                log.debug(
                    f"Removed concept ({concept.id} | {concept.name}): concept name exists in record"
                )
            else:
    ...
    def add_dosages_to_concepts(
        ...
        for ind, concept in enumerate(concepts):
            ...
            if len(dosage_string.split()) > 2:
                ...
                concept.category = (
                    Category.MEDICATION if concept.dosage is not None else None
                )
    ...
    def _process_meta_annotations(self, concept: Concept) -> Optional[Concept]:
        ...
        if convert:
            ...
            if Presence.SUSPECTED in meta_ann_values:
                log.debug(
                    f"Removed concept ({concept.id} | {concept.name}): suspected with no conversion match"
                )
                return None
            if Relevance.IRRELEVANT in meta_ann_values:
                log.debug(
                    f"Removed concept ({concept.id} | {concept.name}): irrelevant concept"
                )
                return None
            if Relevance.HISTORIC in meta_ann_values:
                log.debug(
                    f"No change to concept ({concept.id} | {concept.name}): historic with no conversion match"
                )
...
class MedsAllergiesAnnotator(Annotator):
    ...
    def _validate_and_convert_substance(self, concept) -> bool:
        ...
        if lookup_result is not None:
            ...
            try:
                concept.category = AllergenType(
                    str(lookup_result["allergenType"]).lower()
                )
                log.debug(
            ...
            log.warning(
                f"No lookup subset found for substance ({concept.id} | {concept.name})"
            )
            return False
    ...
    def _validate_and_convert_reaction(self, concept) -> bool:
        ...
        if lookup_result is not None:
            ...
            log.warning(
                f"Reaction not found in Epic subset conversion for concept {concept.__str__()}"
            )
            return False
    ...
    def _validate_and_convert_concepts(self, concept: Concept) -> Concept:
        ...
        if SubstanceCategory.NOT_SUBSTANCE in meta_ann_values and (
            ReactionPos.BEFORE_SUBSTANCE in meta_ann_values
            or ReactionPos.AFTER_SUBSTANCE in meta_ann_values
        ):
    ...
    @staticmethod
    def _link_reactions_to_allergens(
        concept_list: List[Concept], note: Note, link_distance: int = 5
    ) -> List[Concept]:
        allergy_concepts = [
            concept for concept in concept_list if concept.category == Category.ALLERGY
        ]
        reaction_concepts = [
            concept for concept in concept_list if concept.category == Category.REACTION
        ]
        ...
        for reaction_concept in reaction_concepts:
            ...
            meta_ann_values = (
                ...
                if reaction_concept.meta is not None
                else []
            )
            ...
            for allergy_concept in allergy_concepts:
                ...
                if (
                    ReactionPos.BEFORE_SUBSTANCE in meta_ann_values
                    and allergy_concept.start < reaction_concept.start
                ):
                    continue
                # skip if allergy is before and meta is after_substance
                elif (
                    ReactionPos.AFTER_SUBSTANCE in meta_ann_values
                    and allergy_concept.start > reaction_concept.start
                ):
                    continue
                else:
                    distance = calculate_word_distance(
                        reaction_concept.start,
                        reaction_concept.end,
                        allergy_concept.start,
                        allergy_concept.end,
                        note,
                    )
                    log.debug(
    ...
    def _convert_allergy_severity_to_code(concept: Concept) -> bool:
        ...
        if Severity.MILD in meta_ann_values:
            ...
            concept.linked_concepts.append(
                Concept(id="H", name="High", category=Category.SEVERITY)
            )
        elif Severity.UNSPECIFIED in meta_ann_values:
            return True
        else:
            log.warning(
                f"No severity annotation associated with ({concept.id} | {concept.name})"
            )
            return False
    ...
    def _convert_allergy_type_to_code(self, concept: Concept) -> bool:
        ...
        allergy_type = [
            meta_ann for meta_ann in concept.meta if meta_ann.name == "allergytype"
        ]
        if len(allergy_type) != 1:
            log.warning(
                f"Unable to map allergy type code: allergytype meta-annotation "
        ...
        lookup_combination: Tuple[str, str] = (
        ...
        if allergy_type_lookup_result is not None:
            ...
            log.debug(
                f"Linked allergytype concept ({allergy_type_lookup_result[0]} | {allergy_type_lookup_result[1]})"
                f" to allergen concept ({concept.id} | {concept.name}): valid meta model output + allergytype lookup"
            )
        else:
            log.warning(
                f"Allergen and adverse reaction type combination not found: {lookup_combination}"
            )
    ...
    def convert_VTM_to_VMP_or_text(self, concepts: List[Concept]) -> List[Concept]:
        ...
        med_concepts = [
            concept for concept in concepts if concept.category == Category.MEDICATION
        ]
        ...
        for concept in med_concepts:
            if concept.dosage is not None:
                if concept.dosage.dose:
                    if (
                        concept.dosage.dose.value is not None
                        and concept.dosage.dose.unit is not None
                    ):
                        med_concepts_with_dose.append(concept)

        med_concepts_no_dose = [
            concept for concept in concepts if concept not in med_concepts_with_dose
        ]
        ...
        temp_df = pd.DataFrame(
            {
                ...
                "dose": [
                    float(concept.dosage.dose.value)
                    for concept in med_concepts_with_dose
                ],
                "unit": [
                    concept.dosage.dose.unit for concept in med_concepts_with_dose
                ],
            }
        )
        ...
        merged_df = temp_df.merge(
            self.vtm_to_vmp_lookup, on=["vtmId", "dose", "unit"], how="left"
        )
        ...
        for index, concept in enumerate(med_concepts_with_dose):
            ...
            if not pd.isna(vmp_id):
                ...
                concept.name += (
                    " "
                    + str(int(concept.dosage.dose.value))
                    + str(concept.dosage.dose.unit)
                    + " tablets"
                )
    ...
    def __call__(
        ...
        if (
            dosage_extractor is not None
            and "dosage_extractor" not in self.config.disable
        ):
