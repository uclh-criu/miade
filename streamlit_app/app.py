...
def make_train_data(synth_df, name, labels, synth_add_count):
    return pd.concat(
        [
            synth_df[synth_df[name] == label][: synth_add_count[label]]
            for label in labels
        ],
        ignore_index=True,
    )
...
TRAIN_JSON_OPTIONS = [
    f for f in os.listdir(os.getenv("TRAIN_JSON_DIR")) if ".json" in f
]
...
def present_confusion_matrix(model, data):
    ...
    label_names = [
        name.split()[-1] for name in list(evaluation["confusion matrix"].columns)
    ]
...
def add_metacat_models(
    ...
    save_name = (
        Path(model).stem.rsplit("_", 3)[0]
        + "_w_meta_"
        + datetime.now().strftime("%b_%Y").lower()
    )
    try:
        cat_w_meta.create_model_pack(save_dir_path=out_dir, model_pack_name=save_name)
        st.success(
            "Saved MedCAT modelpack at "
            + out_dir
            + save_name
            + "_"
            + cat_w_meta.config.version["id"]
        )
    except Exception as e:
...
class_weights = st.sidebar.checkbox(
    "Balance class weights? (Experimental)", value=False
)
...
with st.sidebar.form(key="model"):
    ...
    metacat_paths = [
        os.path.join(os.getenv("MODELS_DIR"), "/".join(path.split("/")[-2:]))
        for path in selected_models
    ]
    selected_medcat = st.selectbox(
        "Select MedCAT modelpack to package with:", MEDCAT_OPTIONS
    )
...
with tab1:
    ...
    with col1:
        ...
        train_json_path = st.selectbox(
            "Select annotated training data", TRAIN_JSON_OPTIONS
        )
    ...
    with col2:
        st.markdown(
            "**Visualise** the ratio of real and synthetic in your overall training set:"
        )
        if mc is not None:
            chart_data = get_chart_data(
                category_labels, real_label_counts, synth_add_dict
            )
            st.bar_chart(chart_data, height=500)

            synth_train_df = make_train_data(
                all_synth_df, model_name, category_labels, synth_add_dict
            )
            ...
            with st.spinner("Training MetaCAT..."):
                ...
                if class_weights:
                    ...
                    for label in mc.config.general["category_value2id"].keys():
                        ...
                        class_count = train_count.get(label, 0) + synth_count.get(
                            label, 0
                        )
...
with tab2:
    ...
    with col1:
        ...
        model_path = os.path.join(
            os.getenv("MODELS_DIR", "./"), "/".join(model_path.split("/")[-2:])
        )

        test_path = st.selectbox(
            "Select a test set to evaluate your model against:", TEST_JSON_OPTIONS
        )
...
with tab3:
    ...
    data_path = st.selectbox(
        "Select dataset to interact with:", ["MiADE train data (400)"]
    )
...
with tab4:
    ...
    with col4:
        ...
        selected_medcat = st.selectbox(
            "Select a MedCAT modelpack to load:", MEDCAT_OPTIONS
        )
        medcat_path = os.getenv("SAVE_DIR") + selected_medcat
    ...
    with col5:
        if submit:
            ...
            visualize_ner(
                ...
                title=None,
                show_table=False,
                displacy_options={"colors": {"concept": "#F17156"}},
            )
            st.write(output)
